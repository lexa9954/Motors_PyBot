...
bot = cfg.telebot.TeleBot(cfg.BOT_TOKEN_MOTORS, parse_mode='HTML')
...
def find_by_power_first_step(message):
    ...
    if kw.replace('.','',1).isdigit():
        ...
        bot.register_next_step_handler(msg, find_by_power_second_step, kw)
    else:
        if message.text == '🔙 Назад':
            bot.send_message(message.chat.id, text='Назад', reply_markup = keyboards.kb_main_menu())
        else:
...
def notify_admin(txt=''):
    ...
    while True:
        ...
        txt = cfg.notification_message("motors")
        if txt is None: # Пустое текстовое оповещение
            print('Пустое оповещение main')
            pass
        else:
